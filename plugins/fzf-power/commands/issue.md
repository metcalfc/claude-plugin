---
name: issue
description: Report a bug with fzf-power
...
Report a bug with an fzf-power recipe or pattern. Gather context about the failure, sanitize it, and let the user review before filing.
...
- `fzf --version`
- The fzf command or script that failed
- The error message or unexpected output
- The OS (`uname -s`)
- The terminal (`echo $TERM`)
...
```bash
gh issue list --repo metcalfc/claude-plugin --label "fzf-power,bug" --state open --json number,title --jq '.[] | "#\(.number) \(.title)"'
```
